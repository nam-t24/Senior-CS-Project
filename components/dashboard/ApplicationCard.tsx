...
export default function ApplicationCard({appID, name, amount, description, createdDate}: {appID: number, name: string, amount: number, description: string, createdDate: string}){
      ...
      <div className="flex flex-col justify-between border-2 border-neutral-400 px-4 py-4 rounded-lg bg-[#FFFEFE] 2xl:w-[28rem] w-[20rem] 2xl:mr-6 mr-6 2xl:my-4 my-3 group hover:text-darkmaroon hover:border-darkmaroon transition duration-500">
        <div className="">
          <div className="flex justify-between items-start">
            ...
            <Link
              href={`/dashboard/organization/applications/${appID}`}
              className="flex items-center justify-end text-sm space-x-1 hoverRaise min-w-28"
            >
              <p>View Application</p> <ChevronRightIcon fontSize="inherit" />
            </Link>
          </div>
