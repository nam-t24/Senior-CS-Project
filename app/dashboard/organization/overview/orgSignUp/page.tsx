...
export default function OrgSignUpPage() {
    ...
    const createOrg = async () => {
        ...
        await createOrganization(orgName, orgEmail, orgBio, orgType).then(()=>displaySuccessToast()).catch(error => displayErrorToast(error));
    }
    ...
    return(
        ...
        <div className="bg-lightmaroon/20 2xl:w-[60rem] w-[50rem] 2xl:mt-14 mt-8 rounded-lg 2xl:px-14 px-10 2xl:py-14 py-8 2xl:text-lg animate-in">
            ...
            <button className="block px-3 py-1 border-2 border-darkmaroon text-darkmaroon text-xl rounded-xl 2xl:mt-14 mt-8 hover:bg-darkmaroon/20 transition duration-300" onClick={() => createOrg()}>Create Organization</button>
        </div>
